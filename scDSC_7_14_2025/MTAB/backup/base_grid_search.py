...
def quick_train_config(config, config_id, task_id):
    ...
    kmeans = KMeans(n_clusters=8, n_init=5, random_state=42)  # 减少init加速
...
def run_massive_grid_search(task_id, start_idx, end_idx):
    """运行大规模网格搜索的一部分"""
    ...
    device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
    print(f"🖥️  任务{task_id} 设备: {device}")
    ...
    print(f"🔍 任务{task_id} 搜索范围: 配置{start_idx+1}-{end_idx}")
    print(f"📋 本任务配置数: {len(task_combinations)}")
    
    # 创建结果文件
    timestamp = datetime.datetime.now().strftime("%Y%m%d_%H%M%S")
    job_id = os.environ.get('SLURM_JOB_ID', 'local')
    results_file = f"results/logs/massive_task{task_id}_{job_id}_{timestamp}.csv"
    ...
    for i, combination in enumerate(task_combinations):
        ...
        try:
            ...
            # 进度显示
            if (i+1) % 100 == 0 or result['ari'] > 0.72 or result['nmi'] > 0.65:
                elapsed = time.time() - start_time
                remaining = (len(task_combinations) - i - 1) * (elapsed / (i + 1))
                print(f"任务{task_id} [{i+1:4d}/{len(task_combinations)}] "
                      f"ARI={result['ari']:.4f}, NMI={result['nmi']:.4f}, "
                      f"Score={result['score']:.4f} (剩余:{remaining/60:.1f}min)")
                
                # 发现超级结果时详细显示
                if result['ari'] > 0.72 or result['nmi'] > 0.65:
                    print(f"   🎯🔥 任务{task_id} 发现超级配置: {config}")
        
        except Exception as e:
            print(f"❌ 任务{task_id} 配置 {global_config_id} 失败: {e}")
            continue
    
    # 总结
    total_time = time.time() - start_time
    print(f"\n🏁 任务{task_id} 完成！用时: {total_time/60:.1f}分钟")
    ...
    if results:
        ...
        print(f"\n📈 任务{task_id} 统计:")
        print(f"ARI: 最佳={max(aris):.4f}, 平均={np.mean(aris):.4f}")
        print(f"NMI: 最佳={max(nmis):.4f}, 平均={np.mean(nmis):.4f}")
        print(f"超越论文ARI(0.62): {sum(1 for ari in aris if ari > 0.62)}/{len(aris)}")
        print(f"超越论文NMI(0.68): {sum(1 for nmi in nmis if nmi > 0.68)}/{len(nmis)}")
...
if __name__ == "__main__":
    ...
    # 分配配置范围 (总共11,250个配置)
    total_configs = 11250
    configs_per_task = total_configs // 3
    
    if task_id == 1:
        start_idx, end_idx = 0, 3750
    elif task_id == 2:
        start_idx, end_idx = 3750, 7500
    elif task_id == 3:
        start_idx, end_idx = 7500, 11250
    else:
        print(f"❌ 无效任务ID: {task_id}")
        exit(1)
    
    print(f"🚀 启动任务{task_id}")
    run_massive_grid_search(task_id, start_idx, end_idx)
