...
def test_250_epochs():
    ...
    print(f"📊 数据: {X_scaled.shape}")
    
    # 使用已知最优配置
    config = {
        'alpha': 0.05, 'beta': 0.005, 'gamma': 0.8, 'delta': 0.1,
        'lr': 1.2e-4, 'sigma': 0.5, 'epochs': 250, 'use_scheduler': True
    }
    
    print(f"🔧 测试配置: {config}")
    
    # 创建模型
    model = SDCN_Test(500, 500, 2000, 2000, 500, 500, 5000, 20, 8, v=1, use_zinb=True).to(device)
    ...
    for epoch in range(config['epochs']):
        ...
        if epoch % 20 == 0:
            with torch.no_grad():
                ...
                print(f"Epoch {epoch:3d}: ARI={ari_z:.4f}, NMI={nmi_z:.4f}, 用时={current_time:.1f}s")
    ...
    print(f"⚡ 每轮平均: {training_time/epoch:.2f}秒")
    print(f"🏆 最佳结果: ARI={best_ari:.4f}, NMI={best_nmi:.4f}")
    print(f"📈 论文对比: ARI={best_ari/0.62*100:.1f}%, NMI={best_nmi/0.68*100:.1f}%")
    
    # 估算网格搜索时间
    grid_size = 7680  # 原始网格大小
    estimated_total = (training_time * grid_size) / 3600  # 小时
    print(f"\n🧮 网格搜索时间估算:")
    print(f"   单配置时间: {training_time:.1f}秒")
    print(f"   全网格预计: {estimated_total:.1f}小时")
