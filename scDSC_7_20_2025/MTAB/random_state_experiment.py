...
def run_single_experiment(experiment_id, total_experiments, config, epochs=80):
    ...
    kmeans = KMeans(n_clusters=8, n_init=20, random_state=42, max_iter=300)  # 固定种子
...
def run_random_state_discovery(num_experiments=10):
    """运行随机状态发现实验"""
    ...
    results_file = f"results/logs/random_state_discovery_{timestamp}.pkl"
    ...
    # 保存最佳结果的状态文件
    best_states_file = f"results/logs/best_random_states_{timestamp}.pkl"
...
if __name__ == "__main__":
    ...
    parser.add_argument('--num_experiments', type=int, default=10, help='实验次数')
